...
# Specific files that are definitely our source data
important_sources = ["final_creator_contacts.xlsx", "new_contacts.xlsx"]

for file_path in xlsx_files:
    fname = os.path.basename(file_path).lower()
    is_important = any(k in fname for k in important_sources)
    is_daily = "march" in fname and "emails" in fname
    
    if not (is_important or is_daily):
        continue
...
for date in sorted(signups_by_date.keys(), reverse=True):
    ...
    for email in emails:
        ce = normalize_text(email)
        # Exact match
        if ce in sent_emails:
            matches += 1
            matched_list.append(ce)
        else:
            # Partial match fallback
            found_any = False
            for sent_e in sent_emails:
                if ce in sent_e or sent_e in ce:
                    matches += 1
...
print("\n--- Consolidated Signup Verification (V2: Robust Matching) ---")
